class ManageIQ::Providers::Nutanix::Inventory::Collector::TargetCollection < ManageIQ::Providers::Nutanix::Inventory::Collector
  ...
  def hosts
    ...
    host_records = manager.hosts.where(:ems_ref => references(:hosts)).includes(:ems_cluster)

    clusters_api = NutanixClustermgmt::ClustersApi.new(cluster_mgmt_connection)
    @hosts = host_records.map do |host|
      clusters_api.get_host_by_id(host.ems_cluster.ems_ref, host.ems_ref).data
    end
  end

  def datastores
    return [] if references(:storages).blank?
    ...
    @datastores = references(:storages).map do |datastore_ref|
